...
public class Part2 {
    ...
    public static void findMaxPerm(
        ...
        int[] max, List<List<String>> maxPerm
    ) {
        int recursions = 0;
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (nextDist < MINUTES) {
                recursions++;
                findMaxPerm(g, distances, items, perm1, nextDist, perm2, dist2, max, maxPerm);
            }
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (nextDist < MINUTES) {
                recursions++;
                findMaxPerm(g, distances, items, perm1, dist1, perm2, nextDist, max, maxPerm);
            }
        ...
        if (recursions == 0) {
            ...
            if (value > max[0]) {
                max[0] = value;
                maxPerm.clear();
                maxPerm.add(new ArrayList<>(perm1));
                maxPerm.add(new ArrayList<>(perm2));
            }
            return;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        findMaxPerm(g, dist, significantValves, new ArrayList<>(List.of("AA")), 0, new ArrayList<>(List.of("AA")), 0, max, maxPerm);
